...
async function loadPhotos() {
    try {
        ...
        items.forEach((item, index) => {
            // 使用高清图 src，配合 bloom 效果更好
            loader.load(item.src, (texture) => {
                createPhotoMesh(texture, index, item);
            });
        });
        
        loader.style.display = 'none'; // 隐藏 loading 文字
    } catch (err) {
        console.error(err);
...
function createPhotoMesh(texture, index, itemData) {
    ...
    const mat = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide, transparent: true, opacity: 0.9 });
